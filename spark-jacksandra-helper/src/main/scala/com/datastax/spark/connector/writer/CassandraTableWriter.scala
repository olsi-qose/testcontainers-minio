...
class CassandraTableWriter[T] private(connector: CassandraConnector,
  ...
  def delete(columns: ColumnSelector)(taskContext: TaskContext,
    ...
    writeInternal(getAsyncWriterInternal(deleteQueryTemplate(columns)),
      taskContext,
      data)
  ...
  private def getAsyncWriterInternal(
                                      queryTemplate: String): CassandraAsyncStatementWriter[T] = {
    connector.withSessionDo { session: CqlSession =>
      val protocolVersion = session.getContext.getProtocolVersion
      val stmt = prepareStatement(queryTemplate, session)
      val batchType =
        if (isCounterUpdate) DefaultBatchType.COUNTER
        else DefaultBatchType.UNLOGGED

      val boundStmtBuilder =
        new CassandraBoundStatementBuilder(rowWriter,
          stmt,
          session.getContext,
      ...
      val batchStmtBuilder =
        new BatchStatementBuilder(batchType, writeConf.consistencyLevel)
      val batchKeyGenerator = batchRoutingKey(session) _
      val batchBuilder =
        new GroupingBatchBuilderBase(boundStmtBuilder,
          batchStmtBuilder,
          batchKeyGenerator,
          writeConf.batchSize,
          writeConf.batchGroupingBufferSize)

      val maybeRateLimit: RichStatement => Unit =
        writeConf.throughputMiBPS match {
          case Some(throughput) =>
            val rateLimiter =
              new RateLimiter((throughput * 1024 * 1024).toLong, 1024 * 1024)
            (stmt: RichStatement) =>
      ...
      CassandraAsyncStatementWriter(connector,
        writeConf,
        tableDef,
        stmt,
        batchBuilder,
  ...
  private def writeInternal(
    ...
    logInfo(
      f"Wrote ${rowIterator.count} rows to $keyspaceName.$tableName in $duration%.3f s.")
  }
}

case class CassandraAsyncStatementWriter[T](
...
object CassandraTableWriter {
   ...
   */
  private def checkCollectionBehaviors(
                                        table: TableDef,
                                        columnRefs: IndexedSeq[ColumnRef]): Unit = {
    ...
    val tableListColumnNames = tableCollectionColumns
      ...
      .collect { case (name, _: ListType[_]) => name }

    val tableMapColumnNames = tableCollectionColumns
      .map(c => (c.columnName, c.columnType))
      .collect { case (name, _: MapType[_, _]) => name }
